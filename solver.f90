subroutine solver_acoustic(NumSou,NumRec,nxSou,nySou,nxRec,nyRec,&
S,d,dt,nx,ny,ctmp,nt,Rec_point,&
frames,solver_name,dPML,acc)

implicit none


	!!!! Simulation Geometry
	integer :: i,nt,ny,nx,j,k,frames,PML,iframe
	integer :: snap,store_snap,nt_store,icont,nn,dn
	integer :: NumRec,NumSou,l,m,jj,acc,free_surface
	integer :: dPML,dny,dnx,dny2,nxt,nyt

	integer :: nxSou(NumSou),nySou(NumSou)
	integer :: nxRec(NumRec),nyRec(NumRec)
	integer :: nxreco(NumRec),nyreco(NumRec)

	!!!! Simulation parameters
	real :: dt,d,max,maxvalp3
	real :: S(nt,NumSou),Sadj(nt,NumRec),Rec_point(nt,NumRec)
	real :: ctmp(ny,nx),kernel(ny,nx),kernel2(ny,nx)

	integer, allocatable :: nxs(:),nys(:)
	real, allocatable :: coef1(:),coef2(:)
		
	!!!! Variables ecuacion acustica
	real, allocatable :: pframes(:,:,:)
	...
	character(len=100) :: file_name,solver_name
	character(len=100) :: Strtmp,Strtmp2

!	dPML=25
!	acc=3
!	write(*,*)"PML,acc: ",dPML,acc

	max=-1000
	free_surface=0	!! si =0, PML superior absorbente
	...
	allocate(pframes(frames,nyt,nxt))
	pframes=0.
...
!!!		 Propagation through the medium
		...
		do i=1,NumRec
			Rec_point(k-1,i)=p_3(nyreco(i),nxreco(i))
		enddo
	

	    	if(MOD(k,nn).eq.0)	then

			icont=icont+1
			write(Strtmp2,*) icont

			maxvalp3=maxval(abs(p_3))
			if(maxvalp3.gt.max)	then
				max=maxvalp3
				iframe=icont
				write(*,*) '1. Max value solver snap: ',max
			endif

			pframes(icont,:,:)=p_3(:,:)

			write(*,*) icont*100/frames,'% solver done'

		endif

	enddo

	if(max.gt.1e3)write(*,*)"WARNING: SOLVER NOT WORKING FINE"
	pframes=pframes/max;
	
	write(*,*) '2. Absolute Max value solver and frame: ',max,iframe
	write(*,*) 'Writting normalized solver frames'

	do icont=1,frames

		write(Strtmp2,*) icont

		file_name=trim(adjustl(solver_name)) // trim(adjustl(Strtmp2)) // '.dat'

!		write(*,*)"Create: ",file_name

		open(unit=12,file=file_name,status='unknown') 	
       		do i=1+dnx,dnx+nx
       		do j=1+dny2,dny2+ny
	               	write(12,*) (i-1-dnx)*d,(j-1-dny2)*d,pframes(icont,j,i)
              	enddo
              	enddo
		close(12)
...
end subroutine solver_acoustic
